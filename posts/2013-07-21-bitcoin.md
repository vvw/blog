...
BitCoin的系统是基于**P2P**的设计，配合难以攻破的现代密码系统及开源软件的工作模式。当你想付款时，只需要对方的钱包地址，以及付款金额，你的BitCoin就这么简单的离你而去，更准确的说，是你失去了该BitCoin的所有权。中间没有经过任何代理机构，仅仅经过P2P网络的传输，你的交易就完成了。交易数据保存在世界范围内所有参与BitCoin的系统的数据库中；BitCoin是完全独立存在的，外界无法通过某种核心基础设施来关闭它。世界上所有使用BitCoin既是BitCoin的用户，也是BitCoin的维护者；这一设计保证了BitCoin的安全性和自由性。同时，也可以避免通货膨胀。
...
BitCoin是由P2P网络的节点，在完成一个特定计算问题后产生的。这些计算问题需要大量的计算能力，并且利用[Proof-of-work system](http://en.wikipedia.org/wiki/Proof-of-work)来检验。这一检验系统名为[Hashcash](http://en.wikipedia.org/wiki/Hashcash)，

### Proof-of-work system

这个世界就是这么有意思，计算机擅长计算，人擅长逻辑思维；这就使得一些事情计算机非常擅长，而人不擅长，反之亦是如此。于是在不同场合就有不同的需求。

* RSA Algorithm: 加密者的计算复杂度低，拥有密钥的解密者的计算复杂度亦很低，而不用有密钥的攻击者、破解者计算复杂度是难以接受的
...
计算机的处理速度是我们人类望尘莫及的，这么高效的系统在给人们带来便利的同时，也往往给人们带来烦恼，当下一个典型的问题就是每日成山的垃圾邮件。一台主机1s的时间可以发送上千封垃圾邮件到网络中，为解决类似问题，POW应运而生！
...
说了这么多有趣的东西，还没说到正题；其实Hashcash的原理也很简单：

* Sender: 发送一串含有信息的bit串，首先产生一个随机数$P$，使用某种运算，将之合并并进行Hashing(e.g. SHA-1)；若Hashing后的bit串的头$K$bit全部是0，那么过程结束，发送原始的bit串以及找到的随机数$P$至Recipient；否则，重新生成随机数$P$

* Recipient: 简单的验证收到的bit串和随机数$P$，是否经过Hashing后的bit串的头$K$bit全部是0；若是，则有效

通过数论及密码学设计，可以保证至少存在1个$P \in [1, 2^K]$使得其满足头$K$bit全部是0。而为寻找这一神奇的$P$，我们只能通过BruteForce的方法来确定最终的解。

<br>

不难看出，Hashcash具有如下性质：

1. 其计算复杂度为指数级，需要进行大量的Hashing运算
...
BitCoin是一串数字签名：每一位所有者通过对前一次交易和下一位拥有者的公钥(Public Key)签署一个随机Hash的数字签名(Digital Signature)，并将这个签名附在这枚Coin的末尾，这枚Coin就发送给了下一位所有者；而收款人利用自己的私钥(Private Key)对数字签名进行检验，就可以验证该Coin的有效性。

该过程存在一个严重的问题：双重支付，即这枚Coin的原始拥有者是否将它同时付给了多个人。由于我们不希望引入中央机构，所以唯一的方法就是了解所有交易的历史记录，并且公开宣布所有的交易记录，系统内的所有参与者都公认唯一的交易历史记录，而这一记录则以文件的形式保存，称为block。而一个block之间通过链式(chain)结构相连接，称为block chain。

为了保护block中的数据不易被修改，于是引入了Proof-of-work机制。在产生block的过程中需要大量的Hashing运算，如果你意图篡改新block的数据，那么你所要对抗的是整个P2P网络中所有其它的计算能力，所以只要诚实的节点在网络中占据优势地位，整个网络就能正常运行。

BitCoin使用的POW机制是类似于Hashcase的方式。但是有几点变化：

* Hashcash通常只能将难度(difficulty)翻倍或减半，但BitCoin则有更为复杂的调整策略 

* Hashcash通常使用SHA-1算法，而BitCoin使用SHA256算法

* BitCoin运行Hashing两次以增加安全性

由于Hashing运算需要消耗大量CPU时间，系统会定时发放BitCoin以奖励贡献计算资源的系统维护者，BitCoin就此在网络中产生，这也是BitCoin产生的唯一方式，称为挖矿(Mining)。尽管BitCoin是系统定时产生，但产生的BitCoin总数量是有界的(bounded)。在最初的四年里会有$10,500,000$ BTC被制造出来，这个数字每四年减半，最终BitCoin的数额会无限趋近于$21,000,000$ BTC。更准确的说，BitCoin是随着block的增加线性增长的，从创世block(Genesis block)的$50$ BTC，到每个block $25$ BTC，再到$12.5$ BTC，直到最终收敛。而且，随着时间的增长，网络中的节点数增多，Hashcash的困难程度也指数级的增长，目前的难度已经比创世时，增长了$2^{55} = 3.44 \times 10^{10}$倍。
